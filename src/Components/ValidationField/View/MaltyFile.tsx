...
const MaltyFile = ({ name, label, onChange, isDisabled, placholder, className, props ,no_label}: any) => {
  ...
  const FilehandleChange = ({ file, fileList ,no_label}: any) => {
    formik.setFieldValue(name, fileList.map((file: any) => file.originFileObj));
  };

  const customRequest = async ({ onSuccess ,no_label}: any) => {
